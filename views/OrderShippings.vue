...
import { orderShippingService, type OrderShipping } from '../index'

const router = useRouter()
const orderShippings = ref<OrderShipping[]>([])
...
const fetchOrderShippings = async (params: {
  ...
  try {
    const response = await orderShippingService.getAll({
      page: params.page,
    ...
    orderShippings.value = response.data.data
    pagination.value = response.data.meta
    search.value = params.search ?? search.value
    sort.value = params.sort ?? sort.value
    direction.value = params.direction ?? direction.value
  } catch (err: any) {
    error.value = err.message || 'Hiba történt a szállítási módok betöltése során'
...
onMounted(() => {
  fetchOrderShippings({ page: 1, sort: 'code', direction: 'asc' })
})
</script>

<template>
  <AdminLayout page-title="Szállítási módok">
    <div class="space-y-6">
      <div class="flex items-center justify-between">
        <h1 class="text-3xl font-bold tracking-tight">Szállítási módok</h1>
      </div>

      <Card>
        <CardHeader class="pb-3">
          <CardTitle>Szállítási módok kezelése</CardTitle>
        </CardHeader>
        <CardContent class="space-y-4">
          ...
          <DataTable
            ...
            @fetch="fetchOrderShippings"
          >
            <template #actions>
              <Button class="gap-2" @click="router.push({ name: 'order-shipping.create' })">
                <Plus class="h-4 w-4" />
                Új szállítási mód
              </Button>
            </template>
            ...
            <template #empty>
              Nincs megjeleníthető szállítási mód.
            </template>
          </DataTable>
        </CardContent>
      </Card>
